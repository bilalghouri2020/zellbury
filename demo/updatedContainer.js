 ...
 export class CheckoutContainer extends PureComponent {
     ...
     constructor(props) {
         ...
         this.state = {
             ...
             sourcesCount: 1,
             dropdown: false,
             locationPopup: navigator.geolocation && navigator.geolocation.watchPosition((cords) => {
                 if(cords){
                     console.log('cords', cord);
                     return true
                 }
                 console.log('cords', false);
                 return false
             })
         };
         ...
         if (!navigator.geolocation) {
             ...
             if (this.state.locationPopup) {
                 navigator.geolocation.getCurrentPosition(async ({ coords }) => {
                     ...
                     console.log('location off ho gayi hai');
                     let sourceData;
 
                     if (customer.data) {
                         console.log('customer.data', customer.data);
                         if (customer?.data?.quoteid) {
                             console.log('customer.data.quoteid', customer?.data?.quoteid);
                             ...
                             console.log('ELSE customer.data.quoteid', customer?.data?.quoteid);
                             await this.props.requestCustomerData()
                             const { quoteid } = BrowserDatabase.getItem(CUSTOMER)
                         ...
                         const cloneAddress = { ...address }
                         console.log('after const cloneAddress = { ...address }');
                         const sourcesCount = [...new Set(JSON.parse(data.getSourceList.sourcename))].length;
                         console.log('after const sourcesCount = [...new Set(JSON.parse(data.getSourceList.sourcename))].length;');
                         cloneAddress.city = address.city ? address.city : customer?.data?.city
                         console.log('after cloneAddress.city = address.city ? address.city : customer?.data?.city');
                         fetchMutation(CheckoutQuery.getEstimateShippingCosts(
                             cloneAddress,
     ...
     render() {
         return (
             <Checkout
                 ...
                 {...this.containerProps()}
                 onLocationPopup={this.onLocationPopup.bind(this)}
                 locationState={this.state.locationPopup}
             />
         );
     }
 }
