...
export class ProductListContainer extends PureComponent {
    ...
    componentDidUpdate(prevProps) {
        const { location: { pathname }, isLoading, pages, sort, search, filter } = this.props;
        const { location: { pathname: prevPathname }, pages: oldPages, sort: prevSort, search: prevSearch, filter: prevFilter } = prevProps;
        ...
        const newKey = Object.keys(pages)[0];
        const oldKey = Object.keys(oldPages)[0];

        if(!isLoading) {
            if (pathname !== prevPathname || newKey !== oldKey) {
                const pageItems = pages[Object.keys(pages)[0]];
                let itemsObject = {};

                pageItems.map((items, index) => {
                    const { id, sku, name, price_range } = items;

                    //let pushItem = [{"prodId":id},{"prodSku":sku},{"prodName":name}];
                    let itemPrice = price_range.minimum_price.final_price.value;

                    let pushItem = {"prodId":id, "prodSku":sku, "prodName":name, "prodPrice":itemPrice, "prodCat":pathname};
                    itemsObject[index] = pushItem;
                });
                if(itemsObject) {
                    const GtmDataLayerCategoryArgs = {
                        dataLayer: {
                            pageType: 'category',
                            currencyCode: 'PKR',
                            prodImpressions: itemsObject
                        }
                    };
                    TagManager.dataLayer(GtmDataLayerCategoryArgs);
                }
            }
        }
